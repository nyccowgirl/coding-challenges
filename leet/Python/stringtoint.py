...
-> 32). If no digits were read, then the integer is 0. Change 
the sign as necessary (from step 2).

If the integer is out of the 32-bit signed integer range [-231, 
231 - 1], then clamp the integer so that it remains in the 
range. Specifically, integers less than -231 should be clamped 
to -231, and integers greater than 231 - 1 should be clamped to 
231 - 1.
...
character. Do not ignore any characters other than the leading 
...
Since 42 is in the range [-231, 231 - 1], the final result is 
42.
...
Output: 4193
Explanation:
Step 1: "4193 with words" (no characters read because there is 
...
because the next character is a non-digit)
             ^
The parsed integer is 4193.
Since 4193 is in the range [-231, 231 - 1], the final result is 
4193.
...
Output: 0
Explanation:
Step 1: "words and 987" (no characters read because there is no 
leading whitespace)
         ^
Step 2: "words and 987" (no characters read because there is 
neither a '-' nor '+')
         ^
Step 3: "words and 987" (reading stops immediately because
there is a non-digit 'w')
         ^
...
Output: -2147483648
Explanation:
Step 1: "-91283472332" (no characters read because there is no 
leading whitespace)
         ^
...
Since -91283472332 is less than the lower bound of the range 
[-231, 231 - 1], the final result is clamped to -231 = 
-2147483648.
...
class Solution:
    def myAtoi(self, s: str) -> int:
        MIN, MAX = -2 ** 31, 2 ** 31 - 1
        i, base, sign, length = 0, 0, 1, len(s)
        
        if length > 0:
            while (s[i] == ' ' and i < (length - 1)):
                i += 1
                
            if (s[i] == '+' or s[i] == '-') and i < (length - 1):
